...
function initCarousel() {
  ...
  function applyLayout(dragPx, animated) {
    ...
    const nearest = Math.max(
      0,
      Math.min(TOTAL - 1, Math.round(activeIdx - dragPx / g)),
    );
    dots.forEach((d, i) =>
      d.classList.toggle("c-slide__dot--active", i === nearest),
    );
  }
  ...
  function snapTo(idx, animated = true) {
    ...
    dots.forEach((d, i) =>
      d.classList.toggle("c-slide__dot--active", i === activeIdx),
    );
...
function initTiktokSlide() {
  ...
  if (card9 && antonyEl) {
    const antonyObserver = new IntersectionObserver(
      (entries) => {
